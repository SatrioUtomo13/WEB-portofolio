...
@section('content')
    <section class="w-1/2">
        <h1 class="font-montserrat font-semibold text-4xl text-gray-800 mb-5">Edit Resume</h1>
        ...
        <form action="/dashboard/resume/{{ $resume->id }}" method="POST" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-6">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white" for="file_input">Image</label>
                <input type="hidden" value="{{ $resume->image }}" name="oldImage">
